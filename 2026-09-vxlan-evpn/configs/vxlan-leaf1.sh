#!/bin/bash
# leaf1 -- the VXLAN dataplane. THIS is where VNI 100 comes from; there is no
# 'vni 100' line in the FRR config. zebra discovers this over netlink.
#
# ens19 is the host-facing NIC (toward host-a). Adjust for your hardware.
# MTU 1450 = 1500 underlay - 50 bytes of VXLAN encapsulation.
#
# NOT persistent -- 'write memory' does not save any of this, it is kernel
# state rather than FRR config. See ../systemd/evpn-fabric-leaf1.service.
set -eux

UP=ens19

ip link add vxlan100 type vxlan id 100 dstport 4789 local 192.168.30.11 nolearning
